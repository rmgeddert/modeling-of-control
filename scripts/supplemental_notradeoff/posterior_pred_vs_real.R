...
posterior_notradeoff <- read_csv("model_fitting/NoTradeoff_model/posterior_pred.csv") %>%
  group_by(congruency, taskSequence, switchProp, incProp) %>%
  median_qi(RT)
...
plot_notradeoff <- plot_func(posterior_notradeoff, c("c"="#AA9355", "i"="#843C0C")) +
  theme(legend.position="bottom",
        axis.title.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.text.y = element_blank())
legend <- cowplot::get_legend(plot_notradeoff)
plot_notradeoff <- plot_notradeoff + theme(legend.position="none")

plot(legend)
plot_2d + plot_notradeoff

ggsave("plots/posteriors_plot_notradeoff.jpg", plot_2d + plot_notradeoff, dpi = 300, width = 6.5, height = 4, units = "in")

# ------------------------------------------------
# MAKE LATEX TABLE
# save out latex table of posterior median/95CIs and the real means
posterior_table <- posterior_2d %>% 
  left_join(posterior_notradeoff, by=setdiff(names(posterior_2d), c("RT", ".lower", ".upper")), suffix=c(".2D", ".1D")) %>% 
...
latex_text <- kbl_dat %>% 
  ...
  add_header_above(c("Conditions" = 4, "Real Data" = 1, "2D Model" = 4, "No Tradeoff Model" = 4),
                   bold=T, font_size=12) %>%  
