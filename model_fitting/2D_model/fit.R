...
df <- read_csv("data/dataset1.csv", col_types = cols()) %>% 
  select(trialCount, block, acc, RT, stimCongruency, switchType, subject) %>% 
  mutate(RT = RT/1000,
...
stan_data <- list(
  ...
  RTs = df$RT,
  acc = df$acc,
  incongruencies = df$stimCongruency,
  switches = df$switchType,
  K = 15,
  blockN = df$block,
...
optim_params <- list(
  ...
  b_d = optim_params_df[optim_params_df$variable %in% rep(paste0("b_d[", 1:15, "]"), each=1), ]$estimate,
  b_bs = optim_params_df[optim_params_df$variable %in% rep(paste0("b_bs[", 1:15, "]"), each=1), ]$estimate,
  b_ndt = optim_params_df[optim_params_df$variable %in% rep(paste0("b_ndt[", 1:15, "]"), each=1), ]$estimate,
