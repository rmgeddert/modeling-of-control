...
mcmc_trace(fit$draws(c("Intercept_d", rep(paste0("b_d[", 1:15, "]")))), facet_args=list(ncol=2))
mcmc_trace(fit$draws(rep(paste0("b_bs[", 1:15, "]"), each=1)))
mcmc_trace(fit$draws(rep(paste0("b_ndt[", 1:15, "]"), each=1)))
...
drift_vars <- c("Intercept_d", rep(paste0("b_d[", 1:15, "]")))
bs_vars <- c("Intercept_bs", rep(paste0("b_bs[", 1:15, "]")))
ndt_vars <- c("Intercept_ndt", rep(paste0("b_ndt[", 1:15, "]")))

all_vars <- c(drift_vars, bs_vars, ndt_vars)
all_vars_matrix <- matrix(all_vars, nrow = 6, byrow = TRUE)
all_vars_matrix_inverted = aperm(all_vars_matrix, c(2, 1))
inverted_vars_list <- unlist(apply(all_vars_matrix_inverted, 1, list))

hist_plot <- mcmc_trace(fit$draws(inverted_vars_list), facet_args=list(ncol=6)) +
  ggtitle("2-D Model Parameter Traceplots") +
...
ggsave("plots/2d_traceplot.jpg", hist_plot, dpi = 300, width = 6.5, height = 8, units = "in")
